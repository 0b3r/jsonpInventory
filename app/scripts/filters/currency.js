  ...
  angular.module('currencyFilter', []).
    filter('currency', ['$injector', '$locale', function ( $injector, $locale ) {
      ...
      return function ( amount, currencySymbol, fractionSize, suffixSymbol ) {
        ...
        //if ( angular.isUndefined(textIfZeroOrLess) ) { textIfZeroOrLess = amount; }
...
angular
	.module('jsonpApp')
	.filter('messageIfZero', function () {
		return function (amount, message) {
			var originalAmount = amount;
			if(!angular.isNumber(amount) && (amount !== '')){ 
				amount = Number(amount.replace(/[^0-9\.]+/g, ''));
			}
			if(angular.isUndefined(message)){
				message = 'Call for detail';
			}

			var replaceNumer = ((angular.isNumber(amount)) && (amount <= 0));
